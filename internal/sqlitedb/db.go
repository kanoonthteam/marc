...
// RawMessage mirrors a row in the raw_messages table.
...
// InsertRawMessage inserts a single extracted message into raw_messages.
// Uses INSERT OR IGNORE so re-processing the same event is idempotent.
// Returns the row ID (or 0 if the row already existed).
func (d *DB) InsertRawMessage(ctx context.Context, rm RawMessage) (int64, error) {
	const q = `INSERT OR IGNORE INTO raw_messages
	    (event_id, machine, message_index, role, message_json, captured_at)
	    VALUES (?, ?, ?, ?, ?, ?)`
	res, err := d.db.ExecContext(ctx, q,
		rm.EventID, rm.Machine, rm.MessageIndex, rm.Role,
		rm.MessageJSON, rm.CapturedAt.UTC().Format(time.RFC3339),
	)
	if err != nil {
		return 0, fmt.Errorf("sqlitedb: insert raw_message: %w", err)
	}
	return res.LastInsertId()
}

// UpdateRawMessageDecision sets the has_decision column for a raw_message row.
func (d *DB) UpdateRawMessageDecision(ctx context.Context, id int64, hasDecision bool) error {
	const q = `UPDATE raw_messages SET has_decision = ? WHERE id = ?`
	_, err := d.db.ExecContext(ctx, q, hasDecision, id)
	if err != nil {
		return fmt.Errorf("sqlitedb: update raw_message decision: %w", err)
	}
	return nil
}
