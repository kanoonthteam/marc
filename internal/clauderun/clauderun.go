...
func Run(opts Options) error {
	...
	listenAddr, err := loadListenAddr(opts.ConfigPath)
	if err != nil {
	...
	cmd := exec.Command(resolved, opts.Args...)
	cmd.Env = withEnv(os.Environ(), "ANTHROPIC_BASE_URL", "http://"+listenAddr)
...
// loadListenAddr reads the [proxy].listen_addr from ~/.marc/config.toml.
// Falls back to 127.0.0.1:8082 when the config doesn't exist or doesn't
// specify it — same default the proxy itself uses.
func loadListenAddr(cfgPath string) (string, error) {
	if cfgPath == "" {
	...
	if _, err := os.Stat(cfgPath); err != nil {
		// No config — fall back to default port.
		return "127.0.0.1:8082", nil
	}
	cfg, err := config.LoadClient(cfgPath)
	if err != nil {
		// Don't fail the passthrough on a config-parse error; proxy
		// itself would also fall back to defaults.
		return "127.0.0.1:8082", nil
	}
